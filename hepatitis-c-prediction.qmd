...
kNN <- kNNdist(scaled_data, k = 5)

# Create a dataframe for plotting
kNN_df <- data.frame(
  Points = 1:length(kNN),
  kNN_Distance = sort(kNN)
)

# Generate a ggplot-based k-NN Distance Plot
ggplot(kNN_df, aes(x = Points, y = kNN_Distance)) +
  geom_line(color = "black", size = 1) +
  geom_hline(yintercept = 1, color = "red", linetype = "dashed", size = 1) +
  labs(
    title = "k-NN Distance Plot (k = 5)",
    x = "Points Sorted by Distance",
    y = "5-NN Distance"
  ) +
  annotate(
    "text", x = nrow(kNN_df) * 0.8, y = 1.2, label = "eps = 1",
